...
<section class="metrics">
	{#each [
		{ label: '全部申请', value: applications.length, tone: 'blue' },
		{ label: '待审批', value: counts.pending, tone: 'amber' },
		{ label: '已通过', value: counts.approved, tone: 'green' },
		{ label: '已驳回', value: counts.rejected, tone: 'red' }
	] as metric}
		<div class="panel metric"><span>{metric.label}</span><strong class={`tone-${metric.tone}`}>{metric.value}</strong><small>较上月保持稳定</small></div>
	{/each}
</section>

<section class="panel recent-panel">
	<div class="section-title"><div><h2>最近申请</h2><p>最新提交的差旅申请记录</p></div><a href="/applications">查看全部 →</a></div>
...
<style>
	...
	.metric { padding: 20px; } .metric span, .metric small { display: block; color: #8a95a8; font-size: 12px; } .metric strong { display: block; margin: 12px 0 6px; font-size: 28px; } .metric small { color: #a4adbb; font-size: 11px; }
